...
import { resolveProductCategory, normalizePolicyStatusForCommission } from '../utils/commissionUtils';
...
export async function calculateAndSetCommission(policyInput: any) {
  ...
  let policyGroupName: string | null = null;
  ...
  const productCategory = resolveProductCategory(policyGroupName);
  const normalizedStatus = normalizePolicyStatusForCommission(policy_creation_status);
